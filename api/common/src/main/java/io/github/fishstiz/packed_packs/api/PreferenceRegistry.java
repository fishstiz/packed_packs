...
public interface PreferenceRegistry {
     ...
     */
    <T> Key<T> register(ResourceLocation id, Class<T> type, T defaultValue);

    /**
     * Registers a new preference with a custom deserializer and serializer.
     */
    <T> Key<T> register(ResourceLocation id, Class<T> type, T defaultValue, Function<String, T> deserializer, Function<T, String> serializer);

    /**
     * Registers a new preference with a custom deserializer. The value is serialized using {@link Object#toString()}
     */
    default <T> Key<T> register(ResourceLocation id, Class<T> type, T defaultValue, Function<String, T> deserializer) {
     ...
     */
    default Key<Float> register(ResourceLocation id, float defaultValue) {
     ...
     */
    default Key<Double> register(ResourceLocation id, double defaultValue) {
