    ...
    private record AnchorRenderer(LayoutElement container, List<AbstractWidget> widgets) implements Renderable {
        ...
        public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
            for (AbstractWidget widget : widgets) {
                ...
                if (topDist > 0) {
                    graphics.verticalLine(wCenterX, containerTop, wy, CommonColors.RED);
                    graphics.text(font, String.valueOf(topDist), wCenterX, containerTop + topDist / 2, CommonColors.WHITE);
                }

                int rightDist = containerRight - (wx + ww);
                if (rightDist > 0) {
                    graphics.horizontalLine(wx + ww, containerRight, wCenterY, CommonColors.GREEN);
                    graphics.text(font, String.valueOf(rightDist), wx + ww + rightDist / 2, wCenterY, CommonColors.WHITE);
                }

                int bottomDist = containerBottom - (wy + wh);
                if (bottomDist > 0) {
                    graphics.verticalLine(wCenterX, wy + wh, containerBottom, CommonColors.BLUE);
                    graphics.text(font, String.valueOf(bottomDist), wCenterX, wy + wh + bottomDist / 2, CommonColors.WHITE);
                }
            }
        }
    }
