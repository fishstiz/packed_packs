...
public class FocusPathRenderer extends TestFeatureRenderer {
    ...
    protected int renderFeature(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, int y, float partialTick) {
        ...
        guiGraphics.text(minecraft.font, "Focus Path: ", 0, y, 0xFFFFFFFF);
        return renderFocusPath(guiGraphics, screen.getFocused(), y + minecraft.font.lineHeight, minecraft.font);
    }

    private static int renderFocusPath(GuiGraphicsExtractor guiGraphics, @Nullable GuiEventListener listener, int y, Font font) {
        if (listener == null) return y;

        String name = listener.getClass().getName();
        guiGraphics.fill(0, y, font.width(name), y + font.lineHeight, 0x7FFF0000);
        guiGraphics.text(font, name, 0, y, 0xFFFFFFFF);
