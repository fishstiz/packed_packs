...
public final class PreferenceRegistryTest {
    ...
    private static ResourceLocation appendPath(ResourceLocation parentId, String path) {
        return parentId.withPath(p -> p + "." + path);
    }

    public static void initialize(PackedPacksApi api, List<ResourceLocation> dependencies) {
        TestFeature feature = TestFeature.builder(api.preferences(), "preference_registry", "Preference Registry")
                .after(dependencies)
                .register(api.eventBus());

        ResourceLocation visibleOptionId = appendPath(feature.id(), "visible_option");
