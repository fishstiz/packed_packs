...
public class PackList extends FZAbstractListWidget<PackList.Entry> implements FocusPathProvider, ContainerEventHandlerPatch {
    ...
    private void renderDroppableSlots(GuiGraphics graphics, ActiveAction.Dragging dragging, int mouseX, int mouseY, float partialTick) {
        int x = getX();
        int y = getY();
        ...
        graphics.renderOutline(x, y, width, height, dropColor);
    }

    private void renderDroppableRect(GuiGraphics graphics, ActiveAction.Dragging dragging, int mouseX, int mouseY) {
        if (canDropAt(dragging, mouseX, mouseY, 0)) {
            if (isHovered()) {
                graphics.fill(getX(), getY(), getRight(), getBottom(), dropRectColor);
            }
            graphics.renderOutline(getX(), getY(), getWidth(), getHeight(), dropColor);
        }
    }
    ...
    public class Entry extends FZAbstractListWidget.Entry implements SelectableEntry, ContainerEventHandlerPatch, FZContextMenu.Source, ElementSink {
        ...
        public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
            ...
            if (isExpandFolder(keyCode, modifiers) && entryModel.folder().isPresent() && entryModel.selectedExclusive()) {
                entryModel.openFolder();
                return true;
            }
            if (isTransfer(keyCode, modifiers) && listModel.supportsTransferring()) {
                entryModel.transfer();
                return true;
            }
            if (isMoveDown(keyCode, modifiers) && listModel.supportsReordering()) {
                entryModel.moveDown();
                return true;
            }
            if (isMoveUp(keyCode, modifiers) && listModel.supportsReordering()) {
                entryModel.moveUp();
                return true;
            }
            ...
            if (isDelete(keyCode, modifiers) && entryModel.fileModifiable()) {
                entryModel.delete();
                return true;
            }
            if (isRename(keyCode, modifiers) && entryModel.fileModifiable()) {
