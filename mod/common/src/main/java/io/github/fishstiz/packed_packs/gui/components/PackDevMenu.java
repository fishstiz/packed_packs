...
class PackDevMenu {
    ...
    private static final ResourceLocation EYE_SLASH_SPRITE = PackedPacks.id("icon/eye_slash");
    private static final ResourceLocation X_SQUARE = PackedPacks.id("icon/x_square");
    ...
    private static final ResourceLocation RADIO_GLOBAL = PackedPacks.id("icon/radio_globe");
    private static final ResourceLocation ALIAS_SPRITE = PackedPacks.id("icon/alias");
    ...
    public void render(GuiGraphics graphics, int top, int left, int width) {
        ...
        if (requiredOverride.exists()) {
            ...
            ResourceLocation sprite = required ? LOCK_SPRITE_SMALL : UNLOCK_SPRITE_SMALL;
    ...
    private ResourceLocation getIcon(boolean active, BiPredicate<Profile, String> defaultOption) {
    ...
    private static WidgetElements createIcon(Supplier<ResourceLocation> iconSupplier) {
    ...
    private void updateNonOverrideEntries(FZContextMenu.Collector collector) {
        collector.addEntry(builder -> buildDevEntry(builder)
                .message(Component.translatable("chat.copy"))
