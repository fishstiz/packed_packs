...
public final class PreferenceRegistryImpl implements PreferenceRegistry {
    private final Map<ResourceLocation, PreferenceImpl<?>> options = new Object2ObjectOpenHashMap<>();
    ...
    @Override
    public <T> Preference<T> register(ResourceLocation id, Class<T> type, T defaultValue) {
        return this.register(id, type, defaultValue, null, null);
    }

    @Override
    public Optional<Preference<?>> find(ResourceLocation id) {
        return Optional.ofNullable(this.options.get(id));
    }

    @Override
    @SuppressWarnings("unchecked")
    public <T> Optional<Preference<T>> find(ResourceLocation id, Class<T> type) {
    ...
    private record PreferenceImpl<T>(ResourceLocation id, Preferences.Option<T> delegate) implements Preference<T> {
        @Override
        public Class<T> type() {
    ...
    private static final class TransientPreference<T> implements Preference<T> {
        ...
        TransientPreference(ResourceLocation id, Class<T> type, T defaultValue) {
            this.id = id;
            this.type = type;
