...
public final class PreferenceRegistryImpl implements PreferenceRegistry {
    private final Map<ResourceLocation, Preferences.Option<?>> options = new Object2ObjectOpenHashMap<>();
    ...
    @Override
    public <T> Key<T> register(ResourceLocation id, Class<T> type, T defaultValue) {
    ...
    private <T> void setOrLog(ResourceLocation id, Preferences.@Nullable Option<T> option, T value) {
        if (option == null) {
